...
## 3 元宇宙商业文明 Metaverse Commerce Civilization
...
盲盒购买过程： Blind box purchase process：
...
通过智能合约，实现Bancor自动定价算法，向DAO进行自动化代币发售。
...
使用Bancor自动定价算法，可以实现从支付代币（比如NEW）向发行代币（AND）的单向兑换，且可以通过调节连接权重（connect weight，缩写为CW）系数来实现对定价曲线的灵活定义。
...
##### 兑换公式  Redemption formula

第一种情况，指定支付一定数量的NEW，求可换得的AND数量
...
第二种情况，指定兑换一定数量的AND，求所需支付NEW的数量
...
发行10000 AND，初始定价为 1 AND = 0.5 NEW，采用定价曲线b（CW = 50%），则：

第1次购买，花费10 NEW，根据公式一可计算出，兑换可得19.8039 AND。实际成交价为 1 AND = 0.505 NEW。

第2次购买，要兑换得100 AND，则根据公式二可计算出，需实际花费53.4902 NEW。实际成交价为 1 AND = 0.5349 NEW。
...
1. 用户打开钱包软件并连接
2. 用户输入要投入的NEW数量，或者输入要兑出的AND数量
3. 用户点击兑换按钮
4. 等待区块链确认交易
5. 交易完成，用户钱包中的NEW被扣除，兑得的AND进入用户钱包
...
仙女座分布式商业的消费者就是AT的持有者，AT持有者消费优惠，也可以用AT抵扣（商家定义定价）；
为仙女座定向增加目标用户，将这些目标用户变为AT持有者，作为向仙女座客户（商户）兜售的增量流量来源。
...
- 代币符号：AT。代币名称：Andromeda Token。
- 总量固定，永不增发。
- 总发行量：1000万亿枚。在Newchain上发行。可跨链至ethereum、heco、bsc等链。
...
通过如下几种方式完成AT的分发：
- （20% = 200万亿）NewSwap发行。
- （20% = 200万亿）各类空投，包括向NEW持有者、其他主网或交易所用户，或者分布式商业节点发行，即通过仙女座小程序本身，发行仙女积分。
- （20% = 200万亿）向节点NFT持有者空投。
- （20% = 200万亿）交易挖矿。在仙女座进行交易的买卖双方均可获得AT奖励。
- （20% = 200万亿）锁仓挖矿。比如向矿池中锁入AND，可以挖出AT。
...
交易节点可以通过仙女座提供的解决方案开展自己的商业活动，并开展私域营销。交易节点获取流量的思想转变为利用仙女座经济模型获取更多的忠实用户，节点的每个用户都是私域粉丝，节点自己掌握各自的运营数据，商业活动不在被中心化平台所胁迫。
