...
When deploying a scene, the `scene.json` file must include information about what parcels will be occupied by this scene in the Andverse map. The CLI reads this information from off this field and deploys to those parcels directly.
...
You can set the parcels in your scene by running the `acl coords` command in your scene folder. This is especially useful for large scenes, as you don't need to list every parcel involved.
...
`acl coords <parcel> <parcel> <parcel>`

> Note: The base parcel must be one of the parcels in the scene.

**Non-square scenes**

The above commands all generate rectangular-shaped scenes. Andverse scenes can have L shapes or other configurations. You can generate a larger square with `acl coords` and then manually remove excess parcels from the `scene.json` file.
